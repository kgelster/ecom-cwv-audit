...
3. **Stage 2, compress and attribute.** `merge_findings.py` reduces ~15MB of raw Lighthouse JSON to an ~8KB `summary.md` and a `findings.json`, ranked by estimated milliseconds saved, with medians and the noise band per metric. Parses only the Lighthouse 13 `*-insight` audits (the legacy ids are gone; a parser written from memory returns nothing and looks like a clean site). Flags an LCP phase breakdown that describes a different element than the one that won.
...
5. **Stage 3, prove the fix.** `verify.sh` pushes the working theme to a new **unpublished** theme with the Shopify CLI, re-measures the same URLs with the same harness settings read back from the baseline, and diffs. It cannot publish.
